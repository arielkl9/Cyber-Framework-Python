...
''')
    ...
    def check_url(self,url):
        if self.domain in url and "http" in url:
            ...
            if self.base_domain in url and self.scan_type.findall(url) == [] and tld_count == 1: # "?" not in url and -> only sites
                self.urls_found.add(url)
                if self.show_print:
                    print(f"{len(self.urls_found)}: {url}\n")
                return self.crawl(url)
        elif url[0] == "/" and self.scan_type.findall(url) == []:
            url = url[1:]
            if f"{self.url}{url}" not in self.urls_found:
                if self.get_url(f"{self.url}{url}"):
                    self.urls_found.add(f"{self.url}{url}")
                    if self.show_print:
                        print(f"{len(self.urls_found)}: {self.url}{url}\n")
    ...
    def time_end(self):
        ...
        spm = len(self.urls_found) / elapsed_time * 60
...
f'''\b************************************************************
    ...
    Working Sites: {len(set(self.urls_found))}              
...
''')
        ...
        with open(f"{path}/log.json","w") as json_file:
            json_file.writelines(json.dumps(list(set(self.urls_found)),indent=2))
